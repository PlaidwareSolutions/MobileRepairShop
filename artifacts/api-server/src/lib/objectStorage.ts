...
export class ObjectStorageService {
  ...
  /** Look up a publicly-readable object by its relative path under any configured public dir. */
   ...
   * Returns the signed upload URL plus the relative path callers should store
   * in the DB (e.g. "inventory/<uuid>") and the absolute URL to render.
   */
  async getPublicObjectUploadURL(opts: { subdir?: string } = {}): Promise<{
    uploadURL: string;
    relativePath: string;
    servingUrl: string;
  }> {
    const subdir = (opts.subdir ?? "uploads").replace(/^\/+|\/+$/g, "");
    const [publicDir] = this.getPublicObjectSearchPaths();
    const objectId = randomUUID();
    const relativePath = `${subdir}/${objectId}`;
    ...
    return {
      uploadURL,
      relativePath,
      servingUrl: `/api/storage/public-objects/${relativePath}`,
    };
  }
}
...
async function signObjectURL({
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to sign object URL (status ${response.status}); make sure you're running on Replit`,
    );
  }
