...
All probes below were issued from the api-server container against production on **2026-04-27** between **03:18Z and 03:22Z** UTC.

| Blocker | Probed at (UTC) | Status as of this re-run | Owner / unblocking task |
|---|---|---|---|
| **D1** prod serves stale `MAIL_FROM_EMAIL=hello@nawazcoded.me` | 2026-04-27T03:18Z | **Still active.** `curl -H "x-admin-password: …" https://gadgetxrepairs.com/api/admin/messaging/config` → `{"emailEnabled":true,"smsEnabled":true,"mailFrom":"hello@nawazcoded.me","smsFrom":"+13466236898"}`. Local dev (post-restart) returns `mailFrom:"support@gadgetxrepairs.com"`. So the shared env is right; production has not yet been redeployed. | Task **#54** (in progress) |
| **D2** prod serves stale `SMS_FROM_NUMBER=+13466236898` | 2026-04-27T03:18Z | **Still active.** Same prod response above shows `smsFrom:"+13466236898"`. Local dev returns `smsFrom:"+18443496782"`. Same root cause as D1 — pending prod redeploy. | Task **#54** (in progress, bundled with D1) |
| **D3** Telnyx profile webhook URL | 2026-04-27T03:20Z | **Still fixed.** `GET https://api.telnyx.com/v2/messaging_profiles/40019dcb-dc53-4eb3-a25d-a02d2c922727` returns `webhook_url: "https://gadgetxrepairs.com/api/webhooks/telnyx"`, `webhook_api_version: "2"`, `enabled: true`. No regression. | n/a — fixed |
| **D4** Resend domain `gadgetxrepairs.com` not verified | 2026-04-27T03:22:25Z | **Confirmed still active with fresh evidence.** Direct probe to Resend's send API (`POST https://api.resend.com/emails` with `from: support@gadgetxrepairs.com`, `to: delivered@resend.dev`, subject tagged `[GX TEST 2026-04-27 Task #48 D4 probe]`) returned **HTTP 403** with body `{"statusCode":403,"message":"The gadgetxrepairs.com domain is not verified. Please, add and verify your domain on https://resend.com/domains","name":"validation_error"}`. No `email_id` was issued (Resend rejects at the domain check before the message enters their queue, so no provider credit was consumed). The send-only `RESEND_API_KEY` returns `401 restricted_api_key` on `GET /domains`, so the dashboard list cannot be inspected programmatically — the send-side probe is the authoritative signal. | Task **#54** (operator action — same task that owns the redeploy, since redeploy without a verified domain still produces failing sends) |
| **D5** Telnyx toll-free `+18443496782` not carrier-verified | 2026-04-27T03:20Z | **Still active.** `GET /v2/verified_numbers` returns `{"data":[],"meta":{"total_results":0,...}}`. `GET /v2/verifications/by_phone_number/+18443496782` returns `{"data":[],"meta":{"total_results":0,...}}`. The toll-free is `phone_number_type: toll_free, status: active, tags: []` on the GadgetXRepairs profile — but it has no verification submission, so Aerial / T-Mobile and the other US carriers will continue to return error 40329 on every send. **Did not re-attempt a live SMS** because the failure mode is fully captured in scenario #14/#15 below; another send would only re-confirm error 40329 and consume Telnyx credit. | Task **#55** (in progress — picks a real number for outbound; whichever number is chosen, it still needs TFN/10DLC verification before traffic flows) |
| **Negative probes — Resend bad svix signature** | 2026-04-27T03:19Z | **PASS.** `POST https://gadgetxrepairs.com/api/webhooks/resend` with junk svix headers → `401 {"error":"Invalid signature"}`. | n/a — healthy |
| **Negative probes — Telnyx missing signature headers** | 2026-04-27T03:19Z | **PASS.** `POST .../api/webhooks/telnyx` with no headers → `401 {"error":"Missing signature headers"}`. | n/a — healthy |
| **Negative probes — Telnyx bad ed25519 signature** | 2026-04-27T03:19Z | **PASS.** Same path with a 64-byte all-zero ed25519 signature + fresh timestamp → `401 {"error":"Invalid signature"}`. Confirms `TELNYX_PUBLIC_KEY` is still present and ed25519 verification is intact. | n/a — healthy |
| **Negative probes — Telnyx stale timestamp guard** | 2026-04-27T03:19Z | **PASS.** Same path with `telnyx-timestamp: 1000000000` (year 2001) → `401 {"error":"Stale timestamp"}`. The 5-minute tolerance window in `webhooks.ts` is enforced. | n/a — healthy |
...
### When to flip the verdict to GO

Verdict stays **NO-GO** until **all three** of the following are evidenced (not assumed) in this report:

1. **D4 cleared** — a `POST /api/admin/leads/.../email` against `https://gadgetxrepairs.com` returns `{ok:true, providerMessageId:"<resend-id>", status:"queued"}`, and a Resend webhook subsequently transitions the row to `delivered`.
2. **D5 cleared** — a `POST /api/admin/leads/.../sms` against `https://gadgetxrepairs.com` returns `{ok:true, providerMessageId:"<telnyx-id>", status:"queued"}`, the recipient confirms physical receipt on `+12817451997`, and a Telnyx webhook transitions the row to `delivered`.
3. **D1 + D2 cleared** — `GET https://gadgetxrepairs.com/api/admin/messaging/config` returns the new sender identities (`support@gadgetxrepairs.com`, `+18443496782` or whichever number Task #55 settles on).
...
### Adjacent observation (resolved separately — see replit.md "SMS sender vs. public phone")

- The public site (`artifacts/gadget-x-repairs`) places `tel:`, `sms:`, and `wa.me` links pointing at the Houston local number `+13466236898`, but that number is not owned by the Telnyx account. The decision (recorded in `replit.md`) is to keep `+13466236898` as the customer-facing display number for local-business / GBP / NAP reasons, and to send outbound SMS from the toll-free `+18443496782` (the only Telnyx-owned number on the GadgetXRepairs.com messaging profile). The shared env var `SMS_FROM_NUMBER=+18443496782` and the `artifacts/api-server/src/lib/messaging.ts` default both reflect this. Customers will therefore see admin-Composer replies arrive from a different number than the one printed on the site. To collapse the two numbers in the future, port `+13466236898` into Telnyx, attach it to profile `40019dcb-...`, then update both `SMS_FROM_NUMBER` and the marketing-site links.
