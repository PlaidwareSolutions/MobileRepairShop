        ...
        <div className="max-w-[1240px] mx-auto grid lg:grid-cols-2 gap-12">
          <div>
            <h2 className="text-3xl md:text-5xl font-black uppercase tracking-tighter mb-6 text-white">
              FROM <span className="text-yellow-400 text-stroke-black">{data.city.toUpperCase()}</span>
            </h2>
            <p className="text-lg font-bold text-zinc-300 mb-6 flex items-start gap-3">
            ...
            <div className="flex flex-wrap gap-2 mb-8">
              {data.landmarks.map((l) => (
                <span key={l} className="bg-zinc-900 border-2 border-zinc-800 px-3 py-1 font-bold uppercase text-xs">
                  {l}
                </span>
              ))}
            </div>
            ...
            <ul className="space-y-3 mb-8">
              {data.whyUs.map((w) => (
                <li key={w} className="flex items-start gap-3 text-base font-bold text-zinc-300">
