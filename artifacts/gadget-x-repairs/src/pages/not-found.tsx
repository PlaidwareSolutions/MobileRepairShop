    ...
    <PageShell hideTicker>
      ...
      <section className="py-24 px-4 bg-zinc-50 min-h-[60vh]">
        <div className="max-w-2xl mx-auto text-center">
          ...
          <h1 className="text-5xl md:text-7xl font-black uppercase tracking-tighter text-zinc-900 mb-4">404</h1>
          ...
          <div className="grid sm:grid-cols-2 gap-3 mb-10 text-left">
            {QUICK_LINKS.map((q) => (
              <Link key={q.to} href={q.to} className="bg-white border-2 border-zinc-300 p-4 hover:border-red-500 transition-colors flex justify-between items-center">
                <span className="font-black uppercase text-sm text-zinc-900">{q.label}</span>
                <ArrowRight className="w-4 h-4 text-red-500" />
              </Link>
            ))}
          </div>
          <Button asChild className="rounded-none bg-red-500 hover:bg-white hover:text-black text-zinc-900 font-black uppercase tracking-widest h-12 px-8">
